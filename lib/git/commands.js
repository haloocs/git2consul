...
async function getExecOptions(cwd) {
  const config = await config_manager.getConfig();
  const execOptions = { cwd };
  if (config.max_buffer_size) {
    execOptions.maxBuffer = config.max_buffer_size;
  }
  return execOptions;
}
