  ...
  this.pull(function(err, output) {
    const errCheckPromise = !err ? 
      ...
      Promise.resolve().then(() => {
        ...
        return this_obj.startFromScratch(err, false);
      });

    errCheckPromise.then(() => {
...
Branch.prototype.startFromScratch = function(err, resetRef, success, failure) {
  var this_obj = this;
  const sanitizedSuccess = success || (arg => arg);
  ...
  return new Promise((resolve, reject) => {
    ...
    this_obj.purgeBranchCache(function(purge_err) {
      ...
      const refProcessingAction = resetRef ?
      // Reset the most recently processed ref: we don't know the state of the repo, so pretend
      // we've never seen this repo before.
        (cb) => consul_broker.setLastProcessedRef(this_obj, null, cb) :
        (cb) => cb();
      refProcessingAction(function(set_last_processed_err) {
        if (set_last_processed_err) return reject(set_last_processed_err);
        ...
        mkdirp(this_obj.branch_parent, function(mkdir_err) {
          /* istanbul ignore next */
          if (mkdir_err) return reject(mkdir_err);
  
          // Clone the branch
          this_obj.clone(function(clone_err) {
            if (clone_err) return reject(clone_err);
  ...
  .finally(() => this_obj.busy = false)
  .then(sanitizedSuccess)
    ...
    if (stat) {
      ...
      return this_obj.getCurrentRef(function(err, ref) {
        if (err) {
          ...
          return this_obj.startFromScratch(null, true, cb, cb);
        }
      ...
      this_obj.startFromScratch(null, true, cb, cb);
    }
  });
};
