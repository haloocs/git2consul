...
const config_reader = require('./config_reader.js');
const config_seeder = require('./config_seeder.js');
...
var read_config = function(cliArgs){
  config_reader.read({'key': global.config_key}, async function(err, config) {
    ...
    if (cliArgs.maxRetries) {
      config.max_retries = cliArgs.maxRetries;
    }
    if (cliArgs.localStore) {
      config.local_store = cliArgs.localStore;
    }
    if (cliArgs.failureSwitch) {
      config.failure_switch = cliArgs.failureSwitch;
    }
    ...
    await config_manager.setConfig(config);
