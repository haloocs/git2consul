...
var process_records = async function(branch, records, cb) {
  ...
  async.eachLimit(records, maxParallelism, (record, asyncCb) => {
    retry((retryCb, retryNum) => {
      return handleRecord(branch, record, records).catch(err => {
    ...
    }, { retries: MAX_OPERATION_RETRIES }).catch(err => {
