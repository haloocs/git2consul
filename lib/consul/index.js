...
const { getConsul } = require('./consul_factory');
...
var write_content_to_consul = function(key_name, content, cb) {
  ...
  getConsul().kv.set({'key': key_name, value: content !== null ? String(content) : null, token: token}, function(err) {
    if (err) {
...
var delete_from_consul = function(key_name, cb) {
  ...
  getConsul().kv.del({'key': key_name, token: token}, function(err) {
    if (err) {
...
var file_deleted = function(branch, file, cb) {
  ...
  getConsul().kv.del({'key': key_name, token: token, recurse: branch.expand_keys}, function(err) {
    /* istanbul ignore if */
...
var get_kvs = function(branch, file, cb) {
  ...
  getConsul().kv.get({'key': key_name + '/', token: token, recurse: true}, function(err, kvs, res) {
 ...
 */
var process_records = async function(branch, records, cb) {
  var errors_seen = [];

  const config = await config_manager.getConfig();
  const maxParallelism = config.max_parallelism || defaults.MAX_PARALLELISM;
  const retryConfig = getRetryConfig(config);
  let totalProcessedFiles = 0;
  let startDate = Date.now();
  const recordsCount = records.length;
  async.eachLimit(records, maxParallelism, async (record) => {
    try {
      if (config.failure_switch && await shouldFail(config.failure_switch)) {
        throw new Error('The user has induced a deliberate update error');
      }

      await retryHandleRecord(retryConfig, branch, record, records);
      totalProcessedFiles++;
      if (totalProcessedFiles % 100 === 0) {
        let nowDate = Date.now();
        const elapsedTime = nowDate - startDate;
        const rate = elapsedTime/totalProcessedFiles;
        const totalExpectedTime = rate * recordsCount;
        logger.debug(`Processed ${totalProcessedFiles} at a rate of ${rate} msec per file. Total files: ${recordsCount}.\nSpent time: ${elapsedTime/1000} sec\nTotal expected time: ${totalExpectedTime/1000} sec\nExpected remaining time: ${(totalExpectedTime - elapsedTime)/1000} sec`);
      }
    } catch (err) {
      // At this point the operation has failed more than MAX_OPERATION_RETRIES times so we accept it as part of life's true 
      // tragedies and handle it the best we can (that means logging).
      let pathErr = new Error('path: ' + record.path);
      logger.error(pathErr);
      logger.error(err);
      errors_seen.push(pathErr, err);
    }
  }, (err) => {
    if (err) {
      logger.error(err);
    }
    if (errors_seen.length > 0) {
      logger.error('Errors have occurred, please check the log');
    }
    return errors_seen.length ? cb(errors_seen) : cb()
  });
};

// Used in order to debug Consul operation failures.
function shouldFail(failureSwitchPath) {
  return readFile(failureSwitchPath, 'utf-8')
    .catch(() => '')
    .then(content => content && content[0] === '1');
}

function retryHandleRecord(retryConfig, branch, record, records) {
  return retry(async (retryCb, retryNum) => {
    try {
      return await handleRecord(branch, record, records);
    } catch (err) {
      // The logic in here is that we should retry an operation if it fails. It is retried up to MAX_OPERATION_RETRIES times.
      logger.error(`File: ${record.path} - Error retry #${retryNum} - Error: ${err}`);
      retryCb(err);
    }
  }, retryConfig);
}
...
function getRetryConfig(config) {
  ...
  if (config.max_retry_wait) {
    retryConfig.maxTimeout = config.max_retry_wait;
  }

  return retryConfig;
}

function handleRecord(branch, record, records) {
    ...
    switch (record.type) {
      ...
      case 'T':
        ...
        if (record.path === branch.common_properties) {
          file_modified(branch, record.path, function() {
            branch.listAdditionalPropertyFiles(records, function(err, additionalRecords) {
              ...
              return process_records(branch, additionalRecords, function(errs) {
                return errs ? reject("Some consul updates failed:\n" + errs.join('\n')) : resolve();
              });
            });
          });
        } else {
          return file_modified(branch, record.path, err => err ? reject(err) : resolve());
        }
        break;
      case 'D':
        // Delete file
        return file_deleted(branch, record.path, err => err ? reject(err) : resolve());
      /* istanbul ignore next */
      default:
 ...
 */
function getLastProcessedRef(branch, cb) {
  var key_name = create_key_name(branch, branch.name + '.ref', true);
  getConsul().kv.get({'key': key_name, token: token}, function(err, item) {
    /* istanbul ignore if */
    if (err) return cb(err);
...
exports.handleRefChange = function(branch, cb) {
  ...
  branch.getCurrentRef(function(err, ref) {
    ...
    getLastProcessedRef(branch, function(err, last_processed_ref) {
      /* istanbul ignore if */
      if (err) return cb(err);
