...
var delete_from_consul = function(key_name, opts, cb) {
  logger.trace('Deleting key %s', key_name);
  const sanitizedOpts = opts || {};
  consul.kv.del(Object.assign({}, {'key': key_name, token: token}, sanitizedOpts), function(err) {
    if (err) {
 ...
 */
function create_key_name(branch, file, ref) {
...
var populate_kvs_from_object = function(branch, prefix, obj, existing_kvs, cb) {
  ...
  if (delete_kvs.length > 0) {
    delete_kvs.forEach(function(del) {
      delete_from_consul(del.key, null, do_writes);
    });
  } else {
    do_writes();
  }
...
var get_kvs = function(branch, file, cb) {
  ...
  consul.kv.get({'key': key_name + '/', token: token, recurse: true}, function(err, kvs) {
...
var process_records = async function(branch, records, cb) {
  ...
  async.eachLimit(records, maxParallelism, async (record) => {
    try {
      await retryHandleRecord(config, branch, record, records);
    } catch (err) {
...
function retryHandleRecord(config, branch, record, records) {
...
function getRetryConfig(config) {
  const retryConfig = {
    retries: config.max_retries || defaults.MAX_RETRIES
  };
  ...
  branch.getCurrentRef(function(err, ref) {
    ...
    getLastProcessedRef(branch, function(err, last_processed_ref) {
      ...
      if (!last_processed_ref) {
        ...
        // We delete everything at the root level because although otherwise we'd overwrite everything anyway,
        // we would not be deleting files that already existed and may no longer be in the repository.
        const rootKey = create_key_name(branch, '', last_processed_ref);
        logger.trace(`Deleting ${rootKey} to make sure the repository and consul are in sync.`);
        delete_from_consul(rootKey, { recurse: true }, (deletionErr) => {
          if (deletionErr) {
            logger.error(deletionErr);
          }
